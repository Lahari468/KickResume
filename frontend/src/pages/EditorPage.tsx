...
const EditorPage: React.FC = () => {
  ...
  // Load template from query param when component mounts or query changes
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const templateIdFromURL = params.get('templateId');

    if (templateIdFromURL) {
      const foundTemplate = templates.find((tpl) => tpl.id === templateIdFromURL);
      if (foundTemplate) {
        setSelectedTemplate(foundTemplate);
        console.log('Template set from URL:', foundTemplate.id);
      }
    }
  }, [location.search, setSelectedTemplate]);

  // Load resume data when resumeId is available
  useEffect(() => {
    const fetchResume = async () => {
      if (resumeId) {
        ...
        try {
          ...
          // Only set template from resume data if no template is selected from URL
          const params = new URLSearchParams(location.search);
          const templateIdFromURL = params.get('templateId');

          // Only use saved template if no URL template AND no currently selected template
          if (!templateIdFromURL && !selectedTemplate && data.template) {
            const templateFromResume = templates.find((tpl) => tpl.id === data.template);
            if (templateFromResume) {
              setSelectedTemplate(templateFromResume);
              console.log('Template set from saved resume:', templateFromResume.id);
            }
          }
    ...
    fetchResume();
  }, [resumeId, selectedTemplate, location.search, setResumeData, setSelectedTemplate]);
